...
```bash
lakeforge agent tasks status
```

To specify a feature explicitly:

```bash
lakeforge agent tasks status --feature 012-documentation-mission
```
...
```python
from lakeforge_cli.agent_utils.status import show_kanban_status
