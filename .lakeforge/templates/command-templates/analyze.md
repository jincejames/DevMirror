...
scripts:
  sh: lakeforge agent check-prerequisites --json --require-tasks --include-tasks
  ps: lakeforge agent -Json -RequireTasks -IncludeTasks
---
...
- **constitution.md** – Project principles (from `.lakeforge/memory/constitution.md`)
...
**Before this**: `/tasks` created your task breakdown (tasks.md complete)
...
- Proceed to `/implement` when ready
- Or return to `/plan` or `/tasks` if major revisions needed
