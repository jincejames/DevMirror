...
This guide helps AI agents use correct PowerShell syntax when working with lakeforge workflows.
...
## Running Lakeforge Commands (PowerShell)

### Using the lakeforge CLI

Lakeforge uses a Python CLI that works across all platforms:

**Common commands:**
- `lakeforge agent feature create-feature <slug>` - Create a new feature
- `lakeforge verify-setup` - Check environment and paths
- `lakeforge agent workflow implement <WPID> --agent <name>` - Start implementing a work package
- `lakeforge agent workflow review <WPID> --agent <name>` - Start reviewing a work package
- `lakeforge agent tasks move-task <WPID> --to for_review` - Complete implementation (move to review)
- `lakeforge merge` - Merge completed feature
...
**Create feature:**
```powershell
.\.lakeforge\scripts\powershell\Create-NewFeature.ps1 `
...
**Check prerequisites:**
```powershell
.\.lakeforge\scripts\powershell\check-prerequisites.ps1 -Json -IncludeTasks
```

**Move task to review (after implementation):**
```powershell
# Using the CLI (recommended):
lakeforge agent tasks move-task WP01 --to for_review --note "Ready for review"

# Or using PowerShell script:
.\.lakeforge\scripts\powershell\Move-TaskToLane.ps1 `
  -Feature "001-auth" `
  -TaskId "WP01" `
  -Lane "for_review" `
  -ShellPid $PID `
...
Note: Git commands work with forward slashes, but native PowerShell file operations expect backslashes. The lakeforge CLI handles this automatically.
...
**Setting variables:**
```powershell
$env:SPEC_KITTY_TEMPLATE_ROOT = "C:\path\to\lakeforge"
```
...
**Using lakeforge commands:**
All lakeforge commands work the same way on PowerShell and Bash:
```powershell
lakeforge agent workflow implement WP01 --agent claude  # Auto-moves to doing
lakeforge agent tasks move-task WP01 --to for_review    # Completion step
lakeforge verify-setup
lakeforge dashboard
```
...
4. **"Unable to import mission module"**
   ...
   - Check: `pip show lakeforge-cli`
   - Fix: Reinstall or check virtual environment

---

**For full lakeforge documentation, see the main templates and README.**