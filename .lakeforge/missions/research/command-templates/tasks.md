...
# Command Template: /tasks (Research Mission)
...
**Note**: Task generation in main is standard for all lakeforge missions. Implementation happens in per-WP worktrees.

---

## Outline

1. **Setup**: Run `lakeforge agent feature check-prerequisites --json --paths-only --include-tasks`

   **CRITICAL**: The command returns JSON with `FEATURE_DIR` as an ABSOLUTE path (e.g., `/Users/robert/Code/project/lakeforge-specs/015-research-topic`).
...
lane: "planned"  # DO NOT EDIT - use: lakeforge agent tasks move-task <WPID> --to <lane>
assignee: ""
agent: ""
shell_pid: ""
review_status: ""
...
**DO NOT use markdown status fields like `**Status**: planned`**. The `lane:` field in YAML frontmatter is the ONLY status tracking mechanism. The review command (`/review`) searches for `lane: "for_review"` in frontmatter to find WPs ready for review.
...
- Use `lakeforge agent tasks move-task` to change lanes
