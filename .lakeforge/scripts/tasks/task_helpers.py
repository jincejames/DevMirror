#!/usr/bin/env python3
"""Standalone helpers for Lakeforge task prompt management."""
...
# IMPORTANT: Keep in sync with src/lakeforge_cli/tasks_support.py
...
def is_legacy_format(feature_path: Path) -> bool:
    ...
    Args:
        feature_path: Path to the feature directory (e.g., lakeforge-specs/007-feature/)
...
def find_repo_root(start: Optional[Path] = None) -> Path:
    ...
    for candidate in [current, *current.parents]:
        ...
        # Also check for .lakeforge marker (fallback for non-git scenarios)
        if (candidate / ".lakeforge").exists():
            return candidate

    raise TaskCliError("Unable to locate repository root (missing .git or .lakeforge).")
...
def detect_conflicting_wp_status(
    ...
    base_path = Path("lakeforge-specs") / feature / "tasks"
...
def locate_work_package(repo_root: Path, feature: str, wp_id: str) -> WorkPackage:
    ...
    if not candidates:
        raise TaskCliError(f"Work package '{wp_id}' not found under lakeforge-specs/{feature}/tasks.")
    if len(candidates) > 1:
