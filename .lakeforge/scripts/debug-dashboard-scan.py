...
from lakeforge_cli.dashboard.scanner import scan_all_features, gather_feature_paths
...
def main():
    ...
    if not feature_paths:
        ...
        print(f"  Main specs: {project_dir / 'lakeforge-specs'} exists: {(project_dir / 'lakeforge-specs').exists()}")
        ...
        if (project_dir / '.worktrees').exists():
            for wt_dir in (project_dir / '.worktrees').iterdir():
                if wt_dir.is_dir():
                    wt_specs = wt_dir / 'lakeforge-specs'
                    print(f"    {wt_dir.name}/lakeforge-specs exists: {wt_specs.exists()}")
                    if wt_specs.exists():
