...
# Command Template: /plan (Documentation Mission)
...
**Note**: Planning in main is standard for all lakeforge missions. Implementation happens in per-WP worktrees.
...
1. **Setup**: Run `lakeforge agent feature setup-plan --json` to initialize plan.md
...
4. **Phase 1: Design**
   ...
   **Output**: Generator config snippets in plan.md, templates ready for implementation
   ...
   Outline high-level work packages (detailed in `/tasks`):
...
- Work breakdown will be detailed in `/tasks`
