...
# /implement - Create Workspace for Work Package
...
```bash
lakeforge agent workflow implement WP## --agent __AGENT__
```
...
✓ Implementation complete and tested:
  lakeforge agent tasks move-task WP## --to for_review --note "Ready for review"
```

**Step 2**: Create the workspace (if needed) and implement according to the prompt
```bash
lakeforge implement WP##              # No dependencies (branches from main)
lakeforge implement WP## --base WPXX  # With dependencies (branches from base WP)
```
...
4. ✅ **WP moved to for_review lane**: `lakeforge agent tasks move-task WP## --to for_review --note "Ready for review"`
...
After `/tasks` generates work packages in the main repository:
- Planning artifacts (spec, plan, tasks) are already in main
- Run `lakeforge agent workflow implement WP01 --agent __AGENT__` to get the full prompt
- Run `lakeforge implement WP01` to create a workspace for the first WP
- Run `lakeforge implement WP02 --base WP01` if WP02 depends on WP01
...
```
/specify → Creates spec.md in main
/plan → Creates plan.md in main
/tasks → Creates tasks/*.md in main
```

**Implementation Phase** (creates worktrees on-demand):
```
lakeforge implement WP01 → Creates .worktrees/###-feature-WP01/
lakeforge implement WP02 --base WP01 → Creates .worktrees/###-feature-WP02/
```
...
#   lakeforge agent tasks move-task WP## --to for_review --note "..."

# 3. Create workspace (if not exists)
lakeforge implement WP##              # Or with --base if dependencies
...
- `planned` → Initial state after `/tasks`
...
**Check current lane**:
```bash
grep "^lane:" lakeforge-specs/###-feature/tasks/WP##-*.md
```

## Troubleshooting

**Error: "Base workspace WP01 does not exist"**
- Solution: Implement WP01 first: `lakeforge implement WP01`

**Error: "WP02 has dependencies. Use: lakeforge implement WP02 --base WP01"**
...
- Check: Did you move to for_review? `lakeforge agent tasks move-task WP## --to for_review`
