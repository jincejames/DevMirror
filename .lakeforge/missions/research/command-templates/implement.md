...
|------|----------|--------------|---------|
| **Sprint Planning** | `lakeforge-specs/{{feature_slug}}/research/` | Main repo | Evidence/sources for planning THIS sprint |
...
cat lakeforge-specs/{{feature_slug}}/meta.json | grep deliverables_path
```
...
- `lakeforge-specs/` (reserved for sprint planning)
- `research/` at project root (ambiguous, conflicts with lakeforge-specs/###/research/)
...
```bash
lakeforge agent tasks move-task {{wp_id}} --to for_review --note "Ready for review: <summary>"
```
...
Planning artifacts in `lakeforge-specs/{{feature_slug}}/research/` are:
...
echo '2025-01-25T14:00:00,journal,"Smith, J. (2024). AI Tools. Nature, 10(2), 123.",AI improves productivity 30%,high,Meta-analysis' \
  >> lakeforge-specs/{{feature_slug}}/research/evidence-log.csv
```
...
echo 'S001,"Smith (2024). AI Tools.",https://example.com,2025-01-25,high,reviewed' \
  >> lakeforge-specs/{{feature_slug}}/research/source-register.csv
```

**Why this matters:**
- Schema validation runs during `/review`
...
| **Merges to main** | Yes, via lakeforge merge | Yes, via lakeforge merge |
| **Planning artifacts** | N/A | `lakeforge-specs/.../research/` (in main) |

### Why This Changed

Previously, research artifacts went in `lakeforge-specs/` which is sparse-excluded from worktrees. This meant:
...
### Mistake 1: Putting Deliverables in lakeforge-specs/

**Wrong**:
```bash
# Creating files in planning artifacts location
echo "# Findings" > lakeforge-specs/{{feature_slug}}/findings.md  # BAD!
```
...
# Edit deliverables
# Immediately run:
lakeforge agent tasks move-task {{wp_id}} --to for_review  # BAD! Nothing committed!
```
...
lakeforge agent tasks move-task {{wp_id}} --to for_review  # GOOD!
```
...
Since deliverables go in `{{deliverables_path}}`, not `lakeforge-specs/`:
...
- [ ] All subtasks marked as done: `lakeforge agent tasks mark-status T### --status done`
