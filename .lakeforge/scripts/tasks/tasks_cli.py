#!/usr/bin/env python3
"""CLI utilities for managing Lakeforge work-package prompts and acceptance."""
...
def _collect_summary_with_encoding(
    ...
    *,
    ...
    try:
        ...
        if cleaned:
            print("[lakeforge] Normalized artifact encoding for:", file=sys.stderr)
            for path in cleaned:
                try:
            ...
            print(
                "[lakeforge] normalize-encoding enabled but no files required updates.",
                file=sys.stderr,
            )
        return collect_feature_summary(
            repo_root,
...
def _check_legacy_format(feature: str, repo_root: Path) -> bool:
    ...
    feature_path = repo_root / "lakeforge-specs" / feature
    if is_legacy_format(feature_path):
        if not _legacy_warning_shown:
            ...
            print("Run `lakeforge upgrade` to migrate to frontmatter-only lanes.", file=sys.stderr)
            print("", file=sys.stderr)
...
def update_command(args: argparse.Namespace) -> None:
    ...
    if _check_legacy_format(feature, repo_root):
        ...
        print("Run 'lakeforge upgrade' first, then retry.", file=sys.stderr)
        sys.exit(1)
...
def list_command(args: argparse.Namespace) -> None:
    repo_root = find_repo_root()
    feature_path = repo_root / "lakeforge-specs" / args.feature
...
def _prepare_merge_metadata(
    ...
    feature_dir = repo_root / "lakeforge-specs" / feature
...
def merge_command(args: argparse.Namespace) -> None:
    ...
    if args.push and has_remote:
        ...
        print("[lakeforge] Skipping push: no remote configured.", file=sys.stderr)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Lakeforge task utilities")
