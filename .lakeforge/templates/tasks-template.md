...
**Prompt Files**: Each work package references a matching prompt file in `/tasks/` generated by `/tasks`. Lane status is stored in YAML frontmatter (`lane: planned|doing|for_review|done`). Treat this file as the high-level checklist; keep deep implementation detail inside the prompt files.
