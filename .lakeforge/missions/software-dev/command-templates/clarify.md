...
1. Run `lakeforge agent feature check-prerequisites --json --paths-only` from the repository root and parse JSON for:
   - `FEATURE_DIR` - Absolute path to feature directory (e.g., `/path/to/lakeforge-specs/017-my-feature/`)
   - `FEATURE_SPEC` - Absolute path to spec.md file
   - If command fails or JSON parsing fails, abort and instruct user to run `/specify` first or verify they are in a lakeforge-initialized repository.
