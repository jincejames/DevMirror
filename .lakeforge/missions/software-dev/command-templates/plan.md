...
2. **Setup**: Run `lakeforge agent feature setup-plan --json` from the repository root and parse JSON for:
   - `result`: "success" or error message
   - `plan_file`: Absolute path to the created plan.md
   - `feature_dir`: Absolute path to the feature directory

3. **Load context**: Read FEATURE_SPEC and `.lakeforge/memory/constitution.md` if it exists. If the constitution file is missing, skip Constitution Check and note that it is absent. Load IMPL_PLAN template (already copied).
...
5. **STOP and report**: This command ends after Phase 1 planning. Report branch, IMPL_PLAN path, and generated artifacts.
   ...
   **⚠️ CRITICAL: DO NOT proceed to task generation!** The user must explicitly run `/tasks` to generate work packages. Your job is COMPLETE after reporting the planning artifacts.
