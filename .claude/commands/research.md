...
*Path: [.lakeforge/templates/commands/research.md](.lakeforge/templates/commands/research.md)*
...
When you run `lakeforge research`, the following files are generated in your feature directory:
...
**Location**: All files go in `lakeforge-specs/001-feature-name/`

---

## Workflow Context

**Before this**: `/plan` calls this as "Phase 0" research phase
...
- Continue with `/plan` which uses your research to drive technical design
...
2. Run `lakeforge research` to generate the mission-specific research artifacts. (Add `--force` only when it is acceptable to overwrite existing drafts.)
...
5. Summarize open questions or risks at the bottom of `research.md`. These should feed directly into `/tasks` and future implementation prompts.

## Success Criteria

- `lakeforge-specs/<feature>/research.md` explains every major decision with references to evidence.
- `lakeforge-specs/<feature>/data-model.md` lists the entities and relationships needed for implementation.
