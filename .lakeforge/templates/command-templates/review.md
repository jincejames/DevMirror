...
scripts:
  sh: lakeforge agent check-prerequisites --json --include-tasks
  ps: lakeforge agent -Json -IncludeTasks
---
...
5. Decide outcome:
  - **Needs changes**:
     ...
     * Run `lakeforge agent move-task <FEATURE> <TASK_ID> planned --note "Code review complete: [brief summary of issues]"` (use the PowerShell equivalent on Windows) so the move and history update are staged consistently.
  - **Approved**:
...
7. Update `tasks.md` automatically:
   - Run `lakeforge agent mark-status --task-id <TASK_ID> --status done` (POSIX) or `lakeforge agent -TaskId <TASK_ID> -Status done` (PowerShell) from repo root.
...
Context for review: {ARGS} (resolve this to the prompt's relative path, e.g., `lakeforge-specs/<feature>/tasks/WPXX.md`)
