...
# You should already be here if you just ran /plan
...
Work packages are generated directly in `lakeforge-specs/###-feature/` and committed to main. Worktrees are created later when implementing each work package.

## Outline

1. **Setup**: Run `lakeforge agent feature check-prerequisites --json --paths-only --include-tasks` from the repository root and capture `FEATURE_DIR` plus `AVAILABLE_DOCS`. All paths must be absolute.

   **CRITICAL**: The command returns JSON with `FEATURE_DIR` as an ABSOLUTE path (e.g., `/Users/robert/Code/new_specify/lakeforge-specs/001-feature-name`).

   **YOU MUST USE THIS PATH** for ALL subsequent file operations. Example:
   ```
   FEATURE_DIR = "/Users/robert/Code/new_specify/lakeforge-specs/001-a-simple-hello"
...
5. **Write `tasks.md`** using the bundled tasks template (`src/specify_cli/missions/software-dev/.lakeforge/templates/tasks-template.md`):
...
6. **Generate prompt files (one per work package)**:
   ...
   - For each work package:
     ...
     - Use the bundled task prompt template (`src/specify_cli/missions/software-dev/.lakeforge/templates/task-prompt-template.md`) to capture:
   ...
   **IMPORTANT**: All WP files live in flat `tasks/` directory. Lane status is tracked ONLY in the `lane:` frontmatter field, NOT by directory location. Agents can change lanes by editing the `lane:` field directly or using `lakeforge agent tasks move-task`.
...
7. **Finalize tasks with dependency parsing and commit**:
   ...
   **CRITICAL**: Run this command from repo root:
   ```bash
   lakeforge agent feature finalize-tasks --json
   ```
...
8. **Report**: Provide a concise outcome summary:
   ...
   - Next suggested command (e.g., `/analyze` or `/implement`)
...
- No dependencies: `lakeforge implement WP01`
- With dependencies: `lakeforge implement WP02 --base WP01`
