...
lane: "planned"  # Edit directly or use: lakeforge agent tasks move-task <WPID> --to <lane>
...
> **Populated by `/review`** – Reviewers add detailed feedback here when work needs changes. Implementation must address every item listed below before returning for re-review.
...
The `lakeforge implement` command will display warnings when:
...
- Link to supporting specs: `.lakeforge/memory/constitution.md`, `lakeforge-specs/.../plan.md`, `lakeforge-specs/.../tasks.md`, data model, contracts, research, quickstart.
...
- Key acceptance checkpoints for `/review`.
...
2. **Use CLI**: `lakeforge agent tasks move-task <WPID> --to <lane> --note "message"`
