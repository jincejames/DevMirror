...
# /specify - Create Research Specification
...
|------|----------|---------|
| **Planning Artifacts** | `lakeforge-specs/###/research/` | Evidence/sources for PLANNING this sprint |
...
> - `docs/<feature-name>/`
> - Custom path (must NOT be inside `lakeforge-specs/`)"

**Default**: If user doesn't specify, use `docs/research/<feature-slug>/`

**Validation Rules**:
- Must NOT be inside `lakeforge-specs/` (reserved for planning artifacts)
...
2. Creates `lakeforge-specs/###-feature/meta.json` with `deliverables_path`
...
The spec template is bundled with lakeforge and will be used automatically. The research mission uses a specialized template optimized for research projects.
